...
import { AlertTriangle, CheckCircle, Clock, Edit2, GripVertical, Loader2, MapPin, Plus, Send, Trash2, Users } from 'lucide-react';
import type { OperationalPeriod, Inject, EOCAction } from '@/lib/utils/ttxGenerator';
...
export function TTXScriptReviewPanel({ script, onSubmit, isSubmitting = false, getSaveKey, openPreviewTab, onPreviewReady, sessionId, onPreviewLoading }: TTXScriptReviewPanelProps) {
  ...
  const handleEditInject = (inject: Inject, periodIdx: number, index: number) => {
  ...
  useEffect(() => {
    ...
    if (!container) return;
    const onScroll = () => {
      const tops = itemRefs.current.map((el) => (el ? el.offsetTop : Number.POSITIVE_INFINITY));
  ...
  const totalInjects = localScript.periods.reduce((sum, p) => sum + p.injects.length, 0);
  const totalActions = localScript.periods.reduce((sum, p) => sum + p.eocActions.length, 0);
  ...
  const reorderInject = (periodIdx: number, fromIndex: number, toIndex: number) => {
  ...
  return (
    <div className="flex grow gap-4">
      ...
      >
        <Card className="bg-background dark:bg-[#18181b] border dark:border-zinc-800">
          <CardContent>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="dark:text-zinc-300">{script.population.toLocaleString()} population</span>
              </div>
            </div>
            <div className="flex items-center gap-4 text-sm pt-2 border-t dark:border-zinc-800">
              <div className="flex items-center gap-2">
                ...
                <span className="dark:text-zinc-300">{script.periods.length} Operational Periods</span>
              </div>
              <div className="flex items-center gap-2">
                <AlertTriangle className="h-4 w-4 text-muted-foreground dark:text-zinc-400" />
                <span className="dark:text-zinc-300">{totalInjects} Injects</span>
              </div>
              <div className="flex items-center gap-2">
          ...
          <div ref={contentRef} className="h-[500px] pr-4 overflow-y-auto">
            <Accordion type="multiple" defaultValue={localScript.periods.map(p => p.id)} className="space-y-2">
                ...
                >
                  <AccordionTrigger className="hover:no-underline">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex items-center gap-2 ml-auto mr-2">
                        ...
                        <Badge variant="secondary" className="dark:bg-zinc-800 dark:text-zinc-300">{period.eocActions.length} actions</Badge>
                      </div>
                    </div>
                  </AccordionTrigger>
                  <AccordionContent className="space-y-4 pt-4">
                    ...
                    <div
                      ...
                      className={`rounded-md ${draggingType === 'inject' ? 'ring-1 ring-cyan-600/60' : ''}`}
                    >
                      ...
                      {period.injects.length === 0 && (
                        <div className={`p-3 border border-dashed rounded-md text-xs text-zinc-400 dark:border-zinc-700 ${draggingType === 'inject' ? 'bg-cyan-900/10' : ''}`}>
                    ...
                    <div
                      ...
                      className={`rounded-md ${draggingType === 'action' ? 'ring-1 ring-emerald-600/60' : ''}`}
                    >
                      ...
                      {period.eocActions.length === 0 && (
                        <div className={`p-3 border border-dashed rounded-md text-xs text-zinc-400 dark:border-zinc-700 ${draggingType === 'action' ? 'bg-emerald-900/10' : ''}`}>
      ...
      <EditInjectDialog
        open={!!editingInject}
        value={editingInject}
        onOpenChange={(open) => { if (!open) { setEditingInject(null); setEditingInjectInfo(null);} }}
        onSave={(updated) => {
