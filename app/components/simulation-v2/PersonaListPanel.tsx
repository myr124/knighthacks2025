...
export function PersonaListPanel() {
  ...
  const filteredPersonas = useMemo(() => {
    return personas.filter(persona => {
      if (filters.sentiment !== 'all' && persona.sentiment !== filters.sentiment) return false;
      if (filters.location !== 'all' && persona.location !== filters.location) return false;
      if (filters.trustInGovernment !== 'all' && persona.demographics.trustInGovernment !== filters.trustInGovernment) return false;
      if (filters.socialStatus !== 'all' && persona.demographics.socialStatus !== filters.socialStatus) return false;
    ...
    <div className="h-full overflow-hidden flex flex-col">
      ...
      <div className="p-4 border-b bg-accent/20">
        <div className="flex items-center justify-between mb-3">
          <Badge variant="outline">{filteredPersonas.length} / {personas.length} personas</Badge>
        </div>
        ...
        <div className="grid grid-cols-3 gap-1 text-xs">
          {Object.entries(sentimentCounts).map(([sentiment, count]) => (
            <div key={sentiment} className="flex items-center gap-1">
              <div className={`w-2 h-2 rounded-full ${SENTIMENT_COLORS[sentiment as PersonaResponse['sentiment']]}`} />
      ...
      <div className="p-4 border-b space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Select value={filters.trustInGovernment} onValueChange={(v) => setFilters({...filters, trustInGovernment: v})}>
            <SelectTrigger className="h-8 text-xs">
          ...
          <Select value={filters.socialStatus} onValueChange={(v) => setFilters({...filters, socialStatus: v})}>
            <SelectTrigger className="h-8 text-xs">
          ...
          <Select value={filters.needsAssistance} onValueChange={(v) => setFilters({...filters, needsAssistance: v})}>
            ...
            <SelectContent>
              ...
              <SelectItem value="false">No Assistance Needed</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
          ...
          {filteredPersonas.length === 0 ? (
            ...
            filteredPersonas.map((persona) => (
              <Card
                ...
                className={`hover:shadow-md transition-shadow cursor-pointer border-l-4 ${LOCATION_COLORS[persona.location]}`}
                ...
                <CardContent className="p-3">
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-sm truncate">{persona.personaName}</h4>
                      <p className="text-xs text-muted-foreground">{persona.personaType}</p>
                    </div>
                    <Badge className={`${SENTIMENT_COLORS[persona.sentiment]} text-white text-xs ml-2`}>
                      {persona.sentiment}
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-xs mb-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="capitalize">{persona.demographics.race}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-muted-foreground">Income:</span>
                      <span>{getDemographicLabel('socialStatus', persona.demographics.socialStatus).split(' ')[0]}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-muted-foreground">Trust:</span>
                      <span>{getDemographicLabel('trustInGovernment', persona.demographics.trustInGovernment).split(' ')[0]}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    {persona.needsAssistance && (
                      <Badge variant="destructive" className="text-xs">Help Needed</Badge>
                    )}
                  </div>
